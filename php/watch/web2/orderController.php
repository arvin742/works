<?php
	...
	switch($act){
		...
		case 'orderVisitEnd': $o->orderVisitEnd($_POST);break;
	...
	class orderController{
		...
		public function orderVisitEnd($post = ""){
			if(!$post){
				echo "<script>alert('数据不能为空')</script>";
				return false;
			}
			...
			$sql = "UPDATE db_orders_use SET status='$data[status]' WHERE user='$data[ip]' AND pid='$data[pid]' AND place='$data[place]'";
			$result = $d->update($sql);
