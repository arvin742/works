$(function(){
	...
	var indexDOM = function(){
		...
		$bt_btn.eq(0).parent().addClass('active');
	...
	var productDOM = function(otps){
		var $dom = $('.product'),
			...
			$bt_btn = $dom.find('.bottom .btn'),
			$new_bt = $dom.find('.new-bt .over');

		$new_bt.attr('href', 'javascript: void(0);').text('已售完');
	...
	var orderDOM = function(opts){
		...
		var init = function(){
			...
			$('#city-selector').on('choose-district.citypicker', function(event, tagert, storage){
				if(storage.name != '选择区县'){
					var province = $dom.find('.citys .province .selector-name').text().trim(),
						city = $dom.find('.citys .city .selector-name').text().trim();

					var area = province == city ? (province + ' ' + storage.name) : (province + ' ' + city + ' ' + storage.name);
					$dom.find('.citys input').val(area);
				}
		...
		var result = function(data){
			if(data.name == ""){
				tips('收货人不能为空');
				$('.order .sec02 .message .name input').focus();
				return false;
			}
			if(data.phone == ""){
				tips('联系方式不能为空');
				$('.order .sec02 .message .phone input').focus();
				return false;
			}	else{
			...
			if(data.address == ""){
				$('.order .sec02 .message .content input').focus();
				tips('详细地址不能为空');
				return false;
			}

			function tips(info){
				$('.nav-tips .info span').text(info);
				$('.nav-tips').show();
				$('.nav-tips .info').addClass('shake');

				setTimeout(function(){
					$('.nav-tips .info span').text('');
					$('.nav-tips .info').removeClass('shake');
					$('.nav-tips').hide();
				}, 1500);
			}
...
!function (o) {
  ...
  o.fn.toTop = function (t) {
    var i = this, e = o(window), s = o("html, body"), n = o.extend({autohide: !0, offset: 420, speed: 500, position: !0, right: 10, top: 70}, t);
